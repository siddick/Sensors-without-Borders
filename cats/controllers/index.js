    ...
    router.get('/', passport.authenticate('basic'), function (req, res) {
        async.parallel({
            ...
            karmaInstallEarn: function (next) {
                db.Activity.aggregate([{
                    $match: { status: 'Completed', description: 'Sensor Device Deployment Form' }
                }, {
                    $group: {
                        _id: {},
                        karma: { $sum: '$karma' }
            ...
            karmaInstallationPending: function (next) {
                db.Activity.aggregate([{
                    $match: { status: 'Pending', description: 'Sensor Device Deployment Form' }, 
                }, {
                    $group: {
                        _id: {},
                        karma: { $sum: '$karma' }
            ...
            karmaMaintenanceEarn: function (next) {
                db.Activity.aggregate([{
                    $match: { status: 'Completed', description: 'Sensor Device Maintenance Form' }
                }, {
                    $group: {
                        _id: {},
                        karma: { $sum: '$karma' }
            ...
            karmaMaintenancePending: function (next) {
                db.Activity.aggregate([{
                    $match: { status: 'Pending', description: 'Sensor Device Maintenance Form' }, 
                }, {
                    $group: {
                        _id: {},
                        karma: { $sum: '$karma' }
